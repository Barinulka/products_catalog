@foreach ($goods as $good)
    <div class="col">
        ...
        <div class="card-body">
            ...
            <p class="card-text pb-7"><small class="text-muted">{{ $category->name }}</small></p>
            <div class="d-flex justify-content-between align-items-center">
                ...
                @auth
                @if ($good->hasFavorite())
                    <a href="{{ route('favorite.remove', ['good' => $good]) }}" class="add-to-favorite" data-update="{{route('category.view', ['url' => $category->url])}}"><i class="bi bi-heart-fill"></i></a>
                @else
                    <a href="{{ route('favorite.add', ['good' => $good]) }}" class="add-to-favorite" data-update="{{route('category.view', ['url' => $category->url])}}"><i class="bi bi-heart"></i></a>
                @endif
                @endauth
            </div>
        </div>
